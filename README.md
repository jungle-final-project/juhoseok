...
셀프 견적 화면의 상품 사진과 공급업체 표시는 DB에 저장된 `part_external_offers` 캐시를 읽습니다. 사용자 부품 조회 API는 네이버 쇼핑 검색 API를 직접 호출하지 않습니다. 저장소 루트 `.env`에 아래 값을 넣고 API를 재빌드한 뒤, 관리자 갱신 API를 호출해 캐시를 채웁니다.
